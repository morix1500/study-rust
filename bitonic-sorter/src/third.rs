    ...
    F: Fn(&T, &T) -> Ordering,
{
    if x.len() > 1 {
        ...
        // 第2引数がtrueのときはcomparetorで示される順序でソート
        do_sort(&mut x[..mid_point], true, comparator);
        // 第2引数がfalseのときはcomparetorで示される順序とは逆順でソート
        do_sort(&mut x[mid_point..], false, comparator);
        sub_sort(x, forward, comparator)
    }
}

fn sub_sort<T, F>(x: &mut [T], forward: bool, comparetor: &F)
where
    F: Fn(&T, &T) -> Ordering,
{
    if x.len() > 1 {
        compare_and_swap(x, forward, comparetor);
        let mid_point = x.len() / 2;
        sub_sort(&mut x[..mid_point], forward, comparetor);
        sub_sort(&mut x[mid_point..], forward, comparetor);
    }
}

fn compare_and_swap<T, F>(x: &mut [T], forward: bool, comparetor: &F)
where
    F: Fn(&T, &T) -> Ordering,
{
    ...
    for i in 0..mid_point {
        if comparetor(&x[i], &x[mid_point + i]) == swap_condition {
