...
// 位置情報
// 例えば Loc(4, 6) なら入力文字の5文字目から7文字までの区間を表す
...
// アノテーション。値に様々なデータをもたせたもの。ここではLocを持たせている
#[derive(Debug, Clone, PartialEq, Eq, Hash)]
struct Annnot<T> {
    value: T,
    loc: Loc,
...
type LexError = Annnot<LexErrorKind>;
...
fn lex_number(input: &[u8], pos: usize) -> Result<(Token, usize), LexError> {
    ...
    let n = from_utf8(&input[start..end])
        .unwrap() // start..posの構成からfrom_utf8は常に成功するためunwrapしても安全
        .parse()
        .unwrap();
...
// posのバイトが期待するものであれば1バイト消費してposを1つすすめる
...
impl fmt::Display for ParseError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        match self {
            ...
            UnclosedOpenParen(tok) => write!(f, "{}: '{}' is not close", tok.loc, tok.value),
            RedundantExpression(tok) => write!(
                f,
                "{}: expression after '{}' is redudant",
                tok.loc, tok.value
            ),
            Eof => write!(f, "End of file"),
        }
...
impl Interpreter {
    ...
    fn eval_binop(&mut self, op: &BinOp, l: i64, r: i64) -> Result<i64, InterpreterErrorKind> {
        ...
        match op.value {
            ...
            Div => {
                if r == 0 {
                    Err(InterpreterErrorKind::DevisionByZero)
                } else {
                    Ok(l / r)
                }
            }
...
type InterpreterError = Annnot<InterpreterErrorKind>;
